...
def print_board(pg,ps,axg,axs,vag,vas):
    skakiera=[["  ","  ","  ","  ","  ","  ","  ","  "],
              ...
              ["  ","  ","  ","  ","  ","  ","  ","  "],
              ["  ","  ","  ","  ","  ","  ","  ","  "]]
    skakiera[int(pg-1)][int(ps-1)]="lp"                    #λευκός πύργος
    skakiera[int(axg-1)][int(axs-1)]="la"                   #λευκός αξιωματικός
    skakiera[int(vag-1)][int(vas-1)]="mv"                   #μαύρη βασίλισσα
    return skakiera
...
def axiomatikos(axg,axs,vag,vas):
    kerdise=False
    if ((axs+axg)%2==1)&((vas+vag)%2==1):       #μάυρα τετράγωνα
...
p2=0
paizei=True
#πρώτος παίκτης false 
#δεύτερος παίκτης true 
for i in range (100):
    if (i+1)%2==0:
        paizei=True
    else:
    ...
    while (pg==axg)&(ps==axs):
        axg=grammes()
        axs=sthles()                        #βρίσκω την γραμμή και την στήλη που είνα κάθε πιόνι και κάνω έλεγχο ώστε να είναι διαφορετική η θέση κάθε πιονιού 
    vag=grammes()
    vas=sthles()
    ...
    for i in range (8):                         
        print(skakiera[i])                          #εκτυπώνει κάθε φορά την σκακιέρα 
    print(" ")
    if paizei==False:
        kerdise=pyrgos(pg,ps,vag,vas)           #για τον πύργο
        if kerdise==True:
            p1+=1
        kerdise=axiomatikos(axg,axs,vag,vas)  #για τον αξιοματικό
        if kerdise==True:
            p1+=1
    else:
        kerdise=pyrgos(pg,ps,vag,vas)           #για την βασίλισσα
        if kerdise==False:
